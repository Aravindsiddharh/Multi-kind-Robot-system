...
def generate_launch_description():
    ...
    nav2_launch_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_bcr, 'launch', 'bringup_launch.py')
        ),
        launch_arguments={
            'use_sim_time': use_sim_time,
            'autostart': autostart,
            'map': os.path.join(pkg_bcr, 'config', 'hospital_map.yaml'),
    ...
    map_server_node = Node(
        ...
        name='map_server',
        output='screen',
        parameters=[{'yaml_filename': os.path.join(pkg_bcr, 'config', 'hospital_map.yaml')}],
    )
